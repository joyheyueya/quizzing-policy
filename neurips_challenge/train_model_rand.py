...
if __name__ == "__main__":
    MAX_EPISODE = 1000000
    METHOD = 'rand'
    SAVE_NAME = 'real_' + METHOD + '_volta13'
    MAX_STEP = 10
    VALIDATE = True
    # GAMMA = 1.0
    ...
    fc_net.to(device)
    oprtimizer_fc = optim.Adam(fc_net.parameters(), lr=learning_rate)
    ...
    # def get_question_to_concept_dict(question):
    #     '''
    #     Returns a mapping from question id to level 3 concept (if a question is associated with 
    #     more than 1 level 3 concept, always take the first one)
        
    #     question: the output of get_question_meta_indexed
    #     '''
    #     result = dict()
    #     for i in range(len(question)):
    #         result[question.iloc[i]['QuestionId']] = question.iloc[i]['SubjectIndexLevelThree'][0]
    #     return result
    # question_to_concept_index = get_question_to_concept_dict(question)
    ...
    selected_users_val = random.sample(list(range(615)), 10)
    for episode in range(MAX_EPISODE):
        print('started episode ' + str(episode), datetime.datetime.now())
        selected_users = random.sample(list(range(len(data_full))), 100)
        ...
        for s in range(MAX_STEP):
            ...
            if 'rand' in METHOD:
                ...
                next_questions = fc_net.select_feature_rl(can_query, question_encodings)
        ...
        torch.save(fc_net.state_dict(), 'results/models/' + SAVE_NAME + '/fc_net_' + SAVE_NAME + '_' + currentDT.strftime("%m-%d %H:%M") +'.pt')
