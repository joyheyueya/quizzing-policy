    ...
    def recompute_outgoing(self, normalize=False):
        ...
        for i in range(len(self._factors)):
            ...
            convg = convg and \
                ...
                self.n_opts
        # if self.name == 'a':
        #     print('self._outgoing', self._outgoing)
        #     print()
        return convg
...
class Factor(object):
    ...
    def get_outgoing_for(self, rv):
        ...
        if self.debug:
            assert self._outgoing is not None, 'must call init_lbp() first'
        # print('inside factor get outgoing for')
        for i, r in enumerate(self._rvs):
            if r == rv:
                # print('get outgoing for rv:', self._outgoing[i])
                return self._outgoing[i]
    ...
    def recompute_outgoing(self, normalize=False):
        ...
        incoming = []
        # print('inside factor recompute')
        # print('old_outgoing', old_outgoing)
        ...
        for i, rv in enumerate(self._rvs):
            ...
            # print('all_idx[:i]', all_idx[:i])
            # print('all_idx[i+1:]', all_idx[i+1:])
            # print('axes', axes)
