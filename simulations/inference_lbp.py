...
class Inference_lbp():
    ...
    # def __init__(self, answer, subject, unary_correctness=0.5, binary_influence=0.6, uniform=False):
    #     subject_leaves = subject.loc[subject.Level == 3]
    #     parents = subject_leaves['ParentIndex'].unique()
    ...
    #     if uniform:
    #         for i in range(len(parents)):
    #             subject_family = subject_leaves.loc[subject.ParentIndex == parents[i]]
    #             for s in subject_family['SubjectIndex']:
    #                 g.rv(str(s), 2) 
    #                 g.factor([str(s)], potential=np.array([0.5, 0.5]))
